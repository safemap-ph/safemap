...
| `in_progress`    | Report in progress for public awareness display     | Public     |
...
```
pending_review → in_progress → verified
              ↘ dismissed
              ↘ spam
```

**Public Visibility:** Only reports with status `in_progress` or `verified` are visible in public endpoints (`/api/reports/public`).
