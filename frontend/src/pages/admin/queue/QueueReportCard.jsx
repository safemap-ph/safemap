...
function QueueReportCard({ report, onView }) {
  ...
  const isUrgent = report.is_urgent === true

  return (
    <div
      className={`w-full bg-white rounded-xl shadow-[0px_0px_3px_0px_rgba(0,0,0,0.08)] border-l-[5px] ${isUrgent ? "border-red-500" : badge.border} p-5 relative`}
    >
      {/* Urgent banner */}
      {isUrgent && (
        <div className="flex items-center gap-1.5 mb-2 px-2 py-1 bg-red-50 border border-red-200 rounded-lg w-fit">
          <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
          <span className="text-red-600 text-[9px] font-extrabold font-['DM_Sans'] uppercase tracking-widest">
            Urgent
          </span>
        </div>
      )}
      ...
      <div className="mb-2">
        ...
        <span className="px-2 py-0.5 bg-gray-100 rounded text-gray-500 text-[9px] font-bold font-['DM_Sans'] uppercase">
          {report.category?.replace(/_/g, " ").toUpperCase() || "REPORT"}
        </span>
      </div>

      {/* Location */}
      <div className="text-gray-400 text-[11px] font-normal font-['DM_Sans'] flex items-center gap-1.5 mb-2">
        <MapPin className="w-3 h-3" />
        {report.location?.barangay || report.location?.city || "General Santos City"}
      </div>

      {/* Contact phone — only shown when urgent */}
      {isUrgent && report.contact_phone && (
        <div className="flex items-center gap-1.5 mb-2">
          <Phone className="w-3 h-3 text-red-400" />
          <span className="text-red-500 text-[11px] font-bold font-['DM_Sans']">{report.contact_phone}</span>
        </div>
      )}
        ...
        <div className="flex items-center gap-2">
          {report.status === "pending_review" && (
            <button
              ...
              className={`h-9 px-4 ${isUrgent ? "bg-red-600 hover:bg-red-700" : "bg-[#1f295b] hover:bg-[#151c3d]"} transition-colors rounded-lg flex items-center gap-2`}
            >
